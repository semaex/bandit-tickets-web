  ...
  <div class="SaleStatus">
    ...
    <div v-if="(openDate || closeDate) && showDates" class="SaleStatus-dates">
      <p v-if="openDate && status === 'coming-soon'" class="SaleStatus-date">
        {{ trans('saleStatus.opening') }} {{ openDate }}
      </p>
      <p v-if="closeDate && status === 'on-sale'" class="SaleStatus-date">
        {{ trans('saleStatus.closing') }} {{ closeDate }}
      </p>
    </div>
  </div>
</template>
...
import saleStatusTranslations from './sale-status.i18n.json'
import { translationService } from '../../../services/translation.service'

// Load translations
translationService.addTranslations('saleStatus', saleStatusTranslations)
...
const statusConfig = computed(() => {
  switch (props.status) {
    case 'on-sale':
      return {
        label: trans('saleStatus.on_sale'),
    ...
    case 'coming-soon':
      return {
        label: trans('saleStatus.coming_soon'),
