...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!ticketId) {
      throw new Error('ticketId not found in token')
    }
  } catch (error) {
    throw createError({
      statusCode: 401,
  ...
  try {
    const endpoint = `/api/v1/admission/tickets/${ticketId}/pdf?token=${token}`
    ...
    console.error('[BFF] Error serving ticket PDF from core:', {
      ticketId,
    ...
    throw createError({
      statusCode: 500,
      message: error.message || 'Failed to serve ticket PDF from core'
    })
  }
})
