...
import { searchEventsByPromoter } from '../../../core-model/Event/CoreEvent.services'
...
export default defineEventHandler(async (event): Promise<EventListPage> => {
  const promoterId = getRouterParamOrFail(event, 'promoterId')
  ...
  return handleCoreApiCall(async (): Promise<EventListPage> => {
    ...
    const result = await searchEventsByPromoter(promoterId, criteria)
  ...
  }, 'fetch upcoming events by promoter')
})
